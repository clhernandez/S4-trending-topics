...
public class TwitterAllAdapter extends AdapterApp implements Runnable {
	...
	public void connectAndRead() throws Exception {
		...
		if (!twitter4jPropsFile.exists()) {
			...
			logger.debug("twitter4j properties finded!");
		}
		...
		logger.debug("pre twitterStream");
		TwitterStream twitterStream = new TwitterStreamFactory(cb.build())
				.getInstance();
		logger.debug("Twitter Stream builded");
		...
		logger.debug("twitter status listener builded");
		twitterStream.addListener(statusListener);
		logger.debug("Added listener");
		twitterStream.sample();
		logger.debug("twitter sampled");
		...
		while(true){
			...
			try {
				...
				if(status.isRetweet()){
                   	...
                   	rtCount = (int) status.getRetweetedStatus().getRetweetCount();
					fvCount = ((Tweet) status.getRetweetedStatus()).getFavoriteCount();
                } 
